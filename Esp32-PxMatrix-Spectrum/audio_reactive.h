...
#define I2S_WS 4                // aka LRCL -> Zold - IO4
#define I2S_SD 32               // aka DOUT -> Sarga - IO32
#define I2S_SCK 12              // aka BCLK -> Kek -IO12
...
//This is a custom made fft range for better visual effects. Higher frequencies can't be detected really good if the microphone is not close to the speaker.
void fft60to960Hz() {
    fftCalc[0] = (fftAdd(3, 4)) / 2;
    fftCalc[1] = (fftAdd(4, 4)) / 2;
    fftCalc[2] = (fftAdd(4, 5)) / 2;
    fftCalc[3] = (fftAdd(5, 6)) / 2;
    fftCalc[4] = (fftAdd(6, 7)) / 2;
    fftCalc[5] = (fftAdd(7, 9)) / 2;
    fftCalc[6] = (fftAdd(9, 10)) / 3;
    fftCalc[7] = (fftAdd(10, 12)) / 3;
    fftCalc[8] = (fftAdd(12, 14)) / 3;
    fftCalc[9] = (fftAdd(14, 17)) / 4;
    fftCalc[10] = (fftAdd(17, 20)) / 4;
    fftCalc[11] = (fftAdd(20, 24)) / 4;
    fftCalc[12] = (fftAdd(24, 29)) / 4;
    fftCalc[13] = (fftAdd(29, 35)) / 4;
    fftCalc[14] = (fftAdd(35, 41)) / 4;
    fftCalc[15] = (fftAdd(41, 51)) / 10;
}
...
void FFTcode(void* parameter) {
    ...
    for (;;) {
        ...
        //fft60to960Hz();  //This is a custom made fft range for better visual effects. Higher frequencies can't be detected really good if the microphone is not close to the speaker.
        fft60to5KHz(); //This is the original fft range
